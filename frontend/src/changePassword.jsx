...
const ChangePassword=({setIsLoggedIn})=>{
    ...
    const UpdatePass=()=>{
        if(setPass.newpassword===setPass.changepassword){
            ...
            if(len_flag && cap_flag && low_flag && num_flag){
                ...
                body: JSON.stringify({token,data:setPass,username:username}),
                }).then((res)=>{
                    if(res.status===200){
    ...
    return (
        <>
        <div className="container-fluid d-flex justify-content-center align-items-center" style={{height: '100vh',background:'gray'}}>
